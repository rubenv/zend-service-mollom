...
class Zend_Service_Mollom_CheckCaptchaTest extends PHPUnit_Framework_TestCase
{
	...
	public function testCheckCaptchaCorrect() {
		try {
			$this->assertTrue($this->mollom->checkCaptcha('test', 'correct'));

		} catch (Zend_Service_Mollom_UnavailableException $e) { /* Might happen */ }
	}

	public function testCheckCaptchaIncorrect() {
		try {
			$this->assertFalse($this->mollom->checkCaptcha('test', 'incorrect'));
